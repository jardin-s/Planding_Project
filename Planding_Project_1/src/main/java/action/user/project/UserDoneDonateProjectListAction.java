...
import svc.user.project.UserDoneDonateProjectListService;
import vo.ActionForward;
import vo.PageInfo;
import vo.ProjectPlannerBean;

public class UserDoneDonateProjectListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		UserDoneDonateProjectListService userDoneDonateProjectListService = new UserDoneDonateProjectListService();
		...
		if(selectOrder != null) {//정렬기준으로 조회
			...
			if(!selectOrder.equalsIgnoreCase("default")) {//혹시모를 default 선택 방지
				...
				listCount = userDoneDonateProjectListService.getDonateProjectDoneCount();
				System.out.println("[UserDoneDonateProjectListAction] project_tbl 진행중인 기부 프로젝트 수 = "+listCount);
				
				//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
				projectPlannerList = userDoneDonateProjectListService.getOrderDonateProjectPlannerDoneList(selectOrder, page, limit);
			...
			//project_tbl에서 기부 프로젝트 수를 얻어옴
			listCount = userDoneDonateProjectListService.getSearchDonateProjectDoneCount(searchTitle);
			System.out.println("[UserDoneDonateProjectListAction] project_tbl 검색조건에 따른 진행중인 기부 프로젝트 수 = "+listCount);
			
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userDoneDonateProjectListService.getSearchDonateProjectPlannerDoneList(searchTitle, page, limit);
			...
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userDoneDonateProjectListService.getDonateProjectPlannerDoneList(page, limit);
		}
		...
		request.setAttribute("showPage", "user/project/userDoneDonateProjectList.jsp");

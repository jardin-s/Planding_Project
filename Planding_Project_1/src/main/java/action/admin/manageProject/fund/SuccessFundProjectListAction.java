...
public class SuccessFundProjectListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(selectOrder != null) {//정렬기준으로 조회
			...
			if(!selectOrder.equalsIgnoreCase("default")) {//혹시모를 default 선택 방지
				...
				listCount = manageStatusProjectListService.getProjectCount("fund", "success");
				System.out.println("[SuccessFundProjectListAction] project_tbl 정렬한 성공 펀딩 프로젝트 수 = "+listCount);
				
				//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
				projectList = manageStatusProjectListService.getOrderProjectList("fund", "success", selectOrder, page, limit);
			...
			//project_tbl에서 기부 프로젝트 수를 얻어옴
			listCount = manageStatusProjectListService.getSearchProjectCount("fund", "success", searchTitle);
			System.out.println("[SuccessFundProjectListAction] project_tbl 검색조건에 따른 성공 펀딩 프로젝트 수 = "+listCount);
			
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectList = manageStatusProjectListService.getSearchProjectList("fund", "success", searchTitle, page, limit);
			...
			//project_tbl에서 기부 프로젝트 수를 얻어옴
			listCount = manageStatusProjectListService.getProjectCount("fund", "success");
			System.out.println("[SuccessFundProjectListAction] project_tbl 성공 펀딩 프로젝트 수 = "+listCount);
			
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectList = manageStatusProjectListService.getProjectList("fund", "success", page, limit);
		}
		...
		request.setAttribute("showAdmin", "admin/manageProject/fundList/successFundProjectList.jsp");

									...
									<td>
										<c:choose>
											...
											<c:when test="${project.p_status eq 'done' && project.curr_amount > 0}">
												<button class="btn btn-outline-primary py-1" type="button" id="refundBtn" onclick="location.href='refundTotalAmount.mngp?project_id=${project.project_id}'">환불</button>
											</c:when>
											<c:otherwise>
		    	...
		    	<div class="row">
					<ul class="pagination justify-content-center">
						<li class="page-item">
							...
							<c:if test="${pageInfo.page > 1}">
								<a class="page-link" href="searchDoneFundProjectList.mngp?page=${pageInfo.page -1 }&title=${searchKeyword}" aria-label="Previous">
									<span aria-hidden="true">&laquo;</span>
								</a>	
							</c:if>								
						</li>
						...
						<c:forEach var="pNum" begin="${pageInfo.startPage }" end="${pageInfo.endPage}" step="1">
							...
							<c:if test="${pNum ne pageInfo.page }">
								<li class="page-item"><a class="page-link" href="searchDoneFundProjectList.mngp?page=${pNum}&title=${searchKeyword}">${pNum}</a></li>
							</c:if>
						</c:forEach>
						...
						<li class="page-item">
							...
							<c:if test="${pageInfo.page < pageInfo.maxPage }">
								<a class="page-link" href="searchDoneFundProjectList.mngp?page=${pageInfo.page +1 }&title=${searchKeyword}" aria-label="Next">
									<span aria-hidden="true">&raquo;</span>
								</a>
							</c:if>							
						</li>

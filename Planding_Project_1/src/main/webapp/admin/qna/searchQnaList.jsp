    		...
    		<div class="d-flex justify-content-end">
				<form action="searchQnaList.adm" method="post" name="fsearch">
	    			<div class="btn btn-outline-light py-1 px-2 me-1">
		    			<input type="text" name="q_title" id="q_title" value="${searchKeyword}" class="border-0 me-2" placeholder="제목으로 검색">
	...
	<c:if test="${pageInfo.listCount == 0 }">
    	<div class="container-xxl py-5">
    		<div class="container col-lg-8 mb-5">
   				<p class="text-center">검색어에 해당하는 문의글이 없습니다.</p>
    		</div>
    	</div>
    </c:if>
	
	<c:if test="${pageInfo.listCount != 0 }">
    	<c:set var="q_index" value="${(pageInfo.page-1)*10 +1}" />
		    	...
		    	<div class="row">
					<ul class="pagination justify-content-center">
						<li class="page-item">
							...
							<c:if test="${pageInfo.page > 1}">
								<a class="page-link" href="searchQnaList.adm?page=${pageInfo.page -1 }&q_title=${searchKeyword}" aria-label="Previous">
									<span aria-hidden="true">&laquo;</span>
								</a>	
							</c:if>								
						</li>
						...
						<c:if test="${pageInfo.page ne null }">
							<c:forEach var="pNum" begin="${pageInfo.startPage }" end="${pageInfo.endPage}" step="1">
								...
								<c:if test="${pNum ne pageInfo.page }">
									<li class="page-item"><a class="page-link" href="searchQnaList.adm?page=${pNum}&q_title=${searchKeyword}">${pNum}</a></li>
								</c:if>
							</c:forEach>
						</c:if>
						...
						<li class="page-item">
							...
							<c:if test="${pageInfo.page < pageInfo.maxPage }">
								<a class="page-link" href="searchQnaList.adm?page=${pageInfo.page +1 }&q_title=${searchKeyword}" aria-label="Next">
									<span aria-hidden="true">&raquo;</span>
								</a>
							</c:if>							
						</li>

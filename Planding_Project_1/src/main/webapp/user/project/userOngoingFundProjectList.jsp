        ...
        <div class="container text-center pt-5">
            <div class="text-center mx-auto" style="max-width: 500px;">
	            ...
	            <ul class="col-12 col-lg-8 nav nav-pills justify-content-center mt-5 mb-0">
					...
					<li class="col-4 nav-item"><a class="nav-link text-white" href="userReadyFundProjectList.pj">공개예정</a></li>
    		...
    		<div class="row">
				...
				<div class="col-4 col-md-3">
					<div class="d-flex justify-content-start">
						<form action="userOngoingFundProjectList.pj" method="post" name="forder">
	    		...
	    		<div class="col auto">
		    		<div class="d-flex justify-content-end">
		    			<form action="userOngoingFundProjectList.pj" method="post" name="fsearch">
    	...
    	<div class="container-xxl py-5">
    		<div class="container col-lg-8 mb-5">
   				<c:if test="${searchKeyword eq null }">
   					<p class="text-center">진행중인 펀딩 프로젝트가 없습니다.</p>
   				</c:if>
				...
				<ul class="pagination justify-content-center">
					...
					<li class="page-item">
						...
						<c:if test="${pageInfo.page > 1}">
							<c:choose>
								<c:when test="${orderKeyword ne null }">
									<a class="page-link" href="userOngoingFundProjectList.pj?page=${pageInfo.page -1 }&selectOrder=${orderKeyword}" aria-label="Previous">
										<span aria-hidden="true">&laquo;</span>
									</a>
								</c:when>
								<c:when test="${searchKeyword ne null }">
									<a class="page-link" href="userOngoingFundProjectList.pj?page=${pageInfo.page -1 }&searchTitle=${searchKeyword}" aria-label="Previous">
										<span aria-hidden="true">&laquo;</span>
									</a>
								</c:when>
								<c:otherwise>
									<a class="page-link" href="userOngoingFundProjectList.pj?page=${pageInfo.page -1 }" aria-label="Previous">
										<span aria-hidden="true">&laquo;</span>
									</a>
								</c:otherwise>
							</c:choose>
					...
					<c:forEach var="pNum" begin="${pageInfo.startPage }" end="${pageInfo.endPage}" step="1">
						...
						<c:if test="${pNum ne pageInfo.page }">
							<c:choose>
								<c:when test="${orderKeyword ne null }">
									<li class="page-item"><a class="page-link" href="userOngoingFundProjectList.pj?page=${pNum}&selectOrder=${orderKeyword}">${pNum}</a></li>
								</c:when>
								<c:when test="${searchKeyword ne null }">
									<li class="page-item"><a class="page-link" href="userOngoingFundProjectList.pj?page=${pNum}&searchTitle=${searchKeyword}">${pNum}</a></li>
								</c:when>
								<c:otherwise>
									<li class="page-item"><a class="page-link" href="userOngoingFundProjectList.pj?page=${pNum}">${pNum}</a></li>
								</c:otherwise>
							</c:choose>
						</c:if>
					</c:forEach>
					...
					<li class="page-item">
						...
						<c:if test="${pageInfo.page < pageInfo.maxPage }">
							<c:choose>
								<c:when test="${orderKeyword ne null }">
									<a class="page-link" href="userOngoingFundProjectList.pj?page=${pageInfo.page +1 }&selectOrder=${orderKeyword}" aria-label="Next">
										<span aria-hidden="true">&raquo;</span>
									</a>
								</c:when>
								<c:when test="${searchKeyword ne null }">
									<a class="page-link" href="userOngoingFundProjectList.pj?page=${pageInfo.page +1 }&searchTitle=${searchKeyword}" aria-label="Next">
										<span aria-hidden="true">&raquo;</span>
									</a>
								</c:when>
								<c:otherwise>
									<a class="page-link" href="userOngoingFundProjectList.pj?page=${pageInfo.page +1 }" aria-label="Next">
										<span aria-hidden="true">&raquo;</span>
									</a>
								</c:otherwise>
							</c:choose>								

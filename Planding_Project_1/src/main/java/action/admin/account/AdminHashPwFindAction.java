...
import svc.admin.account.AdminHashPwFindService;
import util.SHA256;
import vo.ActionForward;
import vo.MemberBean;

public class AdminHashPwFindAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		MemberBean adminInfo = adminHashPwFindService.findHashPw(a_id, a_email);
		...
		if(adminInfo == null) {
			...
			System.out.println("[AdminHashPwFindAction] 생성된 임시비밀번호 random_password = "+random_password);
			
			boolean isSetHashPwSuccess = adminHashPwFindService.setHashPw(a_id, a_email, random_password);
			...
			if(!isSetHashPwSuccess) { //비밀번호 재설정에 실패할 경우
				...
				String receiver = a_email;//받는 사람
				String subject = "[PlanDing] 임시비밀번호 발급";//제목
				...
				final String adminname = "plandingproject"; //구글 아이디 입력
				...
				try {
					...
					Session session = Session.getDefaultInstance(properties,//SMTP서버 정보
																 new Authenticator() {//사용자 인증 정보 객체 : Authenticator추상클래스 생성자() 정의와 동시에 객체생성
																	...
																	protected PasswordAuthentication getPasswordAuthentication() {
																		return new PasswordAuthentication(adminname, password);//"wjddnjs051339","앱비밀번호"
																	}//사용자인증정보 객체를 반환
					...
					System.out.println("AdminPwFindAction : 메일이 정상적으로 전송되었습니다.");//콘솔에 출력 : 메일전송 확인			
					
					
				}catch(Exception e) {
					System.out.println("AdminPwFindAction : SMTP서버가 잘못 설정되었거나 서비스에 문제가 있습니다.");//콘솔에 출력 : 메일전송 확인

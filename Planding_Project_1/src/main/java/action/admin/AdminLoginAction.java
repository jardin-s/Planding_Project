...
import svc.admin.AdminGradeService;
...
public class AdminLoginAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		boolean isLoginSuccess = adminLoginService.login(admin);
		...
		if(!isLoginSuccess) {//거짓 : 로그인 실패하면 (회원이 아니거나 잘못침)
			...
			if(checkbox != null) {//아이디 저장 체크 O
				response.addCookie(cookie);//반드시 마지막에 response에 쿠키를 추가 (클라이언트쪽에 보내기 위해)
			}else {//아이디 저장 체크 X
				cookie.setMaxAge(0);//쿠키를 즉시 삭제 (※-1 : 세션이 끝날 때 삭제)
				response.addCookie(cookie);//반드시 마지막에 response에 쿠키를 추가
			}
			
			//2. 입력한 id로 회원정보를 가져와(이유? session 영역에 공유 - 모든 페이지에서 공유할 수 있음) 
			MemberBean adminInfo = adminLoginService.getAdminInfo(a_id);
						
			//3. "지난달 구매금액"을 조회하여 사용자 등급을 변경 (지난달 구매금액에 따라 등급이 나뉘고, 등급에 따라 가격할인되는 설정)
			//adminInfo = AdminGradeService.updateGrade(adminInfo);
			
			/*4. 로그인에 성공하면 session영역에 속성으로 저장하여 모든 페이지에서 공유
			 * a_id, a_password, a_grade, a_name, a_email
			...
			session.setAttribute("a_grade", adminInfo.getGrade());
			...
			forward = new ActionForward("adminMain.jsp", true);//request에 저장한 게 없으므로 리다이렉션 포워딩
		}

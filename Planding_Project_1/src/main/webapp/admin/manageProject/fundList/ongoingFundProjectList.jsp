        ...
        <div class="container text-center pt-5">
            ...
            <div class="row justify-content-center">
	            <ul class="col-12 col-lg-8 nav nav-pills justify-content-center mt-5 mb-0">
					...
					<li class="col-2 nav-item"><a class="nav-link active fw-bold" aria-current="page" href="#">진행중</a></li>
    		...
    		<div class="row">
				...
				<div class="col-4 col-md-3">
					<div class="d-flex justify-content-start">
						<form action="ongoingFundProjectList.mngp" method="post" name="forder">
	    		...
	    		<div class="col auto">
		    		<div class="d-flex justify-content-end">
		    			<form action="ongoingFundProjectList.mngp" method="post" name="fsearch">
    		...
    		<div class="container col-10 col-md-6 col-lg-4">
    			<div class="col-12 mb-5">
    				...
    				<c:if test="${searchKeyword eq null }">
    					<p class="text-center">진행중인 프로젝트가 없습니다.</p>
    				</c:if>
    			</div>
    		</div>
    	</div>
					...
					<ul class="pagination justify-content-center">
							...
							<c:if test="${pageInfo.page > 1}">
								<c:choose>
									<c:when test="${orderKeyword ne null }">
										<a class="page-link" href="ongoingFundProjectList.mngp?page=${pageInfo.page -1 }&selectOrder=${orderKeyword}" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
									</c:when>
									<c:when test="${searchKeyword ne null }">
										<a class="page-link" href="ongoingFundProjectList.mngp?page=${pageInfo.page -1 }&searchTitle=${searchKeyword}" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
									</c:when>
									<c:otherwise>
										<a class="page-link" href="ongoingFundProjectList.mngp?page=${pageInfo.page -1 }" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
									</c:otherwise>
								</c:choose>
						...
						<c:forEach var="pNum" begin="${pageInfo.startPage }" end="${pageInfo.endPage}" step="1">
							...
							<c:if test="${pNum ne pageInfo.page }">
								<c:choose>
									<c:when test="${orderKeyword ne null }">
										<li class="page-item"><a class="page-link" href="ongoingFundProjectList.mngp?page=${pNum}&selectOrder=${orderKeyword}">${pNum}</a></li>
									</c:when>
									<c:when test="${searchKeyword ne null }">
										<li class="page-item"><a class="page-link" href="ongoingFundProjectList.mngp?page=${pNum}&searchTitle=${searchKeyword}">${pNum}</a></li>
									</c:when>
									<c:otherwise>
										<li class="page-item"><a class="page-link" href="ongoingFundProjectList.mngp?page=${pNum}">${pNum}</a></li>
									</c:otherwise>
								</c:choose>
							</c:if>
						</c:forEach>
						...
						<li class="page-item">
							...
							<c:if test="${pageInfo.page < pageInfo.maxPage }">
								<c:choose>
									<c:when test="${orderKeyword ne null }">
										<a class="page-link" href="ongoingFundProjectList.mngp?page=${pageInfo.page +1 }&selectOrder=${orderKeyword}" aria-label="Next">
											<span aria-hidden="true">&raquo;</span>
										</a>
									</c:when>
									<c:when test="${searchKeyword ne null }">
										<a class="page-link" href="ongoingFundProjectList.mngp?page=${pageInfo.page +1 }&searchTitle=${searchKeyword}" aria-label="Next">
											<span aria-hidden="true">&raquo;</span>
										</a>
									</c:when>
									<c:otherwise>
										<a class="page-link" href="ongoingFundProjectList.mngp?page=${pageInfo.page +1 }" aria-label="Next">
											<span aria-hidden="true">&raquo;</span>
										</a>
									</c:otherwise>
								</c:choose>

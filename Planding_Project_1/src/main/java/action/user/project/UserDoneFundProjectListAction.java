...
import svc.user.project.UserDoneFundProjectListService;
import vo.ActionForward;
import vo.PageInfo;
import vo.ProjectPlannerBean;

public class UserDoneFundProjectListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		UserDoneFundProjectListService userDoneFundProjectListService = new UserDoneFundProjectListService();
		...
		if(selectOrder != null) {//정렬기준으로 조회
			...
			if(!selectOrder.equalsIgnoreCase("default")) {//혹시모를 default 선택 방지
				...
				listCount = userDoneFundProjectListService.getFundProjectDoneCount();
				System.out.println("[UserDoneFundProjectListAction] project_tbl 진행중인 기부 프로젝트 수 = "+listCount);
				
				//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
				projectPlannerList = userDoneFundProjectListService.getOrderFundProjectPlannerDoneList(selectOrder, page, limit);
			...
			//project_tbl에서 기부 프로젝트 수를 얻어옴
			listCount = userDoneFundProjectListService.getSearchFundProjectDoneCount(searchTitle);
			System.out.println("[UserDoneFundProjectListAction] project_tbl 검색조건에 따른 진행중인 기부 프로젝트 수 = "+listCount);
			
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userDoneFundProjectListService.getSearchFundProjectPlannerDoneList(searchTitle, page, limit);
			...
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userDoneFundProjectListService.getFundProjectPlannerDoneList(page, limit);
		}
		...
		request.setAttribute("showPage", "user/project/userDoneFundProjectList.jsp");

...
public class UserMyQnaListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(u_id == null) {//만약 로그인 안 된 상태라면
			...
			/* qna 테이블에서 글을 가져옴 ------------------------------- */
			UserMyQnaListService userMyQnaListService = new UserMyQnaListService();
			
			int listCount = userMyQnaListService.getMyListCount(u_id);//회원의 전체 문의글 개수를 얻어옴
			System.out.println("[UserQnaListAction] "+u_id+"회원의 전체 문의글 개수 = "+listCount);
			
			ArrayList<QnaBean> qnaList = userMyQnaListService.getMyQnaList(u_id, page, limit);//원하는 페이지넘버의 원하는개수만큼 글을 가져옴
			...
			request.setAttribute("showPage", "user/myPage/userMyQnaList.jsp");

...
public class SubmitDonateProjectAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(!isInsertProjectSuccess) {//프로젝트 insert 실패
	        ...
	        //(4) 프로젝트 리워드 insert
	        //기부 프로젝트는 default 리워드만 사용하므로 이 과정이 생략됨
	        
	        //(5) 프로젝트-리워드 매핑 insert
	        boolean isMapProjectReward = insertProjectService.mapProjectReward(project_id, "default");
	        
	        
	        
	        if(!(isInsertPlannerSuccess && isMapProjectReward)) {//기획자 insert와 프로젝트-리워드 매핑 실패시
	        	response.setContentType("text/html; charset=UTF-8");
                ...
                out.println("history.back();");
                out.println("</script>");
	        }else {//기획자 insert와 프로젝트-리워드 매핑 성공시

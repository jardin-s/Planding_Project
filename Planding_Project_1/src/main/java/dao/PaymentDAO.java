...
public class PaymentDAO {
	...
	private static PaymentDAO paymentDAO; //static메서드인 getInstance에서 쓸 수 있게 static (단 외부에서 직접 접근 불가능하도록 private)
	
	public static PaymentDAO getInstance() {
		if(paymentDAO == null) {//DogDAO객체가 없으면
			paymentDAO = new PaymentDAO();//객체 생성
		}
		
		return paymentDAO;//기존 DogDAO객체의 주소 리턴
	}

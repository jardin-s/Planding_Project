...
import svc.user.account.UserLoginService;
import util.SHA256;
...
public class UserLoginAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(a_id != null) {
			...
			out.println("<script>");
			out.println("alert('관리자 로그인된 상태로 사용자 로그인을 할 수 없으므로 자동 로그아웃 처리합니다. 사용자 아이디로 다시 시도해주세요.');");//알림창을 띄우고 로그인폼보기 요청으로 돌아감
			out.println("location.href='adminLogout.adm'");		//(☆history.back()보다는 요청으로 할 것)
			out.println("</script>");
		}else {
			...
			if(!isLoginSuccess) {//거짓 : 로그인 실패하면 (회원이 아니거나 잘못침)
				...
				//만약 로그인 시도한 계정이 관리자 계정이라면
				if(userInfo.getAdmin_status().equalsIgnoreCase("Y")) {
					response.setContentType("text/html; charset=UTF-8");
					
					PrintWriter out = response.getWriter();
					out.println("<script>");
					out.println("alert('관리자 계정으로 접속하실 수 없습니다.');");//알림창을 띄우고 로그인폼보기 요청으로 돌아감
					out.println("location.href='userLoginForm.usr'");		//(☆history.back()보다는 요청으로 할 것)
					out.println("</script>");
				}else {

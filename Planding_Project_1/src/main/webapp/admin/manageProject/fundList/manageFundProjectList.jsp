        ...
        <div class="container text-center pt-5">
            ...
            <div class="row justify-content-center">
	            <ul class="col-12 col-lg-8 nav nav-pills justify-content-center mt-5 mb-0">
					<li class="col-2 nav-item"><a class="nav-link active fw-bold" aria-current="page" href="#">전체</a></li>
    		...
    		<div class="row">
				...
				<div class="col-4 col-md-3">
					<div class="d-flex justify-content-start">
						<form action="manageFundProjectList.mngp" method="post" name="forder">
	    		...
	    		<div class="col auto">
		    		<div class="d-flex justify-content-end">
		    			<form action="manageFundProjectList.mngp" method="post" name="fsearch">
							...
							<c:forEach var="project" items="${projectList}">
								<tr class="text-center">
									...
									<td>
										...
										<c:if test="${project.p_status eq 'done'}">환불요</c:if>
										<c:if test="${project.p_status eq 'clear'}">환불완</c:if>
					...
					<ul class="pagination justify-content-center">
							...
							<c:if test="${pageInfo.page > 1}">
								<c:choose>
									<c:when test="${orderKeyword ne null }">
										<a class="page-link" href="manageFundProjectList.mngp?page=${pageInfo.page -1 }&selectOrder=${orderKeyword}" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
									</c:when>
									<c:when test="${searchKeyword ne null }">
										<a class="page-link" href="manageFundProjectList.mngp?page=${pageInfo.page -1 }&searchTitle=${searchKeyword}" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
									</c:when>
									<c:otherwise>
										<a class="page-link" href="manageFundProjectList.mngp?page=${pageInfo.page -1 }" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
									</c:otherwise>
								</c:choose>
						...
						<c:forEach var="pNum" begin="${pageInfo.startPage }" end="${pageInfo.endPage}" step="1">
							...
							<c:if test="${pNum ne pageInfo.page }">
								<c:choose>
									<c:when test="${orderKeyword ne null }">
										<li class="page-item"><a class="page-link" href="manageFundProjectList.mngp?page=${pNum}&selectOrder=${orderKeyword}">${pNum}</a></li>
									</c:when>
									<c:when test="${searchKeyword ne null }">
										<li class="page-item"><a class="page-link" href="manageFundProjectList.mngp?page=${pNum}&searchTitle=${searchKeyword}">${pNum}</a></li>
									</c:when>
									<c:otherwise>
										<li class="page-item"><a class="page-link" href="manageFundProjectList.mngp?page=${pNum}">${pNum}</a></li>
									</c:otherwise>
								</c:choose>
							</c:if>
						</c:forEach>
						...
						<li class="page-item">
							...
							<c:if test="${pageInfo.page < pageInfo.maxPage }">
								<c:choose>
									<c:when test="${orderKeyword ne null }">
										<a class="page-link" href="manageFundProjectList.mngp?page=${pageInfo.page +1 }&selectOrder=${orderKeyword}" aria-label="Next">
											<span aria-hidden="true">&raquo;</span>
										</a>
									</c:when>
									<c:when test="${searchKeyword ne null }">
										<a class="page-link" href="manageFundProjectList.mngp?page=${pageInfo.page +1 }&searchTitle=${searchKeyword}" aria-label="Next">
											<span aria-hidden="true">&raquo;</span>
										</a>
									</c:when>
									<c:otherwise>
										<a class="page-link" href="manageFundProjectList.mngp?page=${pageInfo.page +1 }" aria-label="Next">
											<span aria-hidden="true">&raquo;</span>
										</a>
									</c:otherwise>
								</c:choose>

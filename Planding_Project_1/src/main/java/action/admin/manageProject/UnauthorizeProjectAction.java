...
public class UnauthorizeProjectAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(a_id == null) {//로그인이 풀린 상태라면
			...
			UnauthorizeProjectService unauthorizeProjectService = new UnauthorizeProjectService();
			
			//프로젝트 ID로 프로젝트 기획자 정보를 얻어옴
			PlannerBean plannerInfo = unauthorizeProjectService.getProjectPlanner(project_id);
			
			//기획자의 회원 ID로 이메일 정보를 얻어옴
			MemberBean plannerUserInfo = unauthorizeProjectService.getPlannerUserInfo(plannerInfo.getMember_id());
			...
			if(plannerInfo == null || plannerUserInfo == null) {//기획자 정보를 얻는 데 실패
				...
				ArrayList<String> rewardList = unauthorizeProjectService.getProjectRewardIdList(project_id);
				...
				if(!isUnauthorizeSuccess) {//모든 삭제 작업에 실패하면
					...
					out.println("<script>");
					out.println("alert('프로젝트 승인 거부가 실패했습니다.');");
					out.println("history.back();");
					out.println("</script>");
					...
					String subject = "[PlanDing] 프로젝트 승인 거절 안내";//제목
					String content = "<p>프로젝트 채택 기준에 따라 심사한 결과 회원님이 신청하신 프로젝트의 승인이 거절되었음을 알려드립니다.</p>";//내용
					...
					try {
						...
						System.out.println("[UnauthorizeProjectAction] 메일이 정상적으로 전송되었습니다.");//콘솔에 출력 : 메일전송 확인			
						
						
					}catch(Exception e) {
						System.out.println("[UnauthorizeProjectAction] SMTP서버가 잘못 설정되었거나 서비스에 문제가 있습니다.");//콘솔에 출력 : 메일전송 확인

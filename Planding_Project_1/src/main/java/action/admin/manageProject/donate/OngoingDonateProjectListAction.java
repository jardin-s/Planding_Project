...
public class OngoingDonateProjectListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if(selectOrder != null) {//정렬기준으로 조회
			...
			if(!selectOrder.equalsIgnoreCase("default")) {//혹시모를 default 선택 방지
				...
				listCount = manageStatusProjectListService.getProjectCount("donate", "ongoing");
				System.out.println("[OngoingDonateProjectListAction] project_tbl 정렬한 진행 기부 프로젝트 수 = "+listCount);
				
				//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
				projectList = manageStatusProjectListService.getOrderProjectList("donate", "ongoing", selectOrder, page, limit);
			...
			//project_tbl에서 기부 프로젝트 수를 얻어옴
			listCount = manageStatusProjectListService.getSearchProjectCount("donate", "ongoing", searchTitle);
			System.out.println("[OngoingDonateProjectListAction] project_tbl 검색조건에 따른 진행 기부 프로젝트 수 = "+listCount);
			
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectList = manageStatusProjectListService.getSearchProjectList("donate", "ongoing", searchTitle, page, limit);
			...
			//project_tbl에서 기부 프로젝트 수를 얻어옴
			listCount = manageStatusProjectListService.getProjectCount("donate", "ongoing");
			System.out.println("[OngoingDonateProjectListAction] project_tbl 진행 기부 프로젝트 수 = "+listCount);
			
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectList = manageStatusProjectListService.getProjectList("donate", "ongoing", page, limit);
		}
		...
		request.setAttribute("showAdmin", "admin/manageProject/donateList/ongoingDonateProjectList.jsp");

...
public class ManageFundProjectListService {
	//필드 없음
	...
	//2. 조건에 맞는 회원 수를 가져옴
	public int getSearchFundCount(String project_title) {
		//1. 커넥션 풀에서 Connection객체를 얻어와
		Connection con = getConnection(); //JdbcUtil. 생략(이유?import static 하여)
	...
	public ArrayList<ProjectBean> getFundList(int page, int limit) {
		//1. 커넥션 풀에서 Connection객체를 얻어와
		Connection con = getConnection(); //JdbcUtil. 생략(이유?import static 하여)
	...
	public ArrayList<ProjectBean> getOrderFundList(String order, int page, int limit) {
		//1. 커넥션 풀에서 Connection객체를 얻어와
		Connection con = getConnection(); //JdbcUtil. 생략(이유?import static 하여)

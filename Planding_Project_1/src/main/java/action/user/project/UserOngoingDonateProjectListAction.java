...
import svc.user.project.UserOngoingDonateProjectListService;
import vo.ActionForward;
import vo.PageInfo;
import vo.ProjectPlannerBean;

public class UserOngoingDonateProjectListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		UserOngoingDonateProjectListService userDonateProjectListService = new UserOngoingDonateProjectListService();
		...
		if(selectOrder != null) {//정렬기준으로 조회
			...
			if(!selectOrder.equalsIgnoreCase("default")) {//혹시모를 default 선택 방지
				...
				listCount = userDonateProjectListService.getDonateProjectOngoingCount();
				System.out.println("[UserOngoingDonateProjectListAction] project_tbl 진행중인 기부 프로젝트 수 = "+listCount);
				
				//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
				projectPlannerList = userDonateProjectListService.getOrderDonateProjectPlannerOngoingList(selectOrder, page, limit);
			...
			//project_tbl에서 기부 프로젝트 수를 얻어옴
			listCount = userDonateProjectListService.getSearchDonateProjectOngoingCount(searchTitle);
			System.out.println("[UserOngoingDonateProjectListAction] project_tbl 검색조건에 따른 진행중인 기부 프로젝트 수 = "+listCount);
			
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userDonateProjectListService.getSearchDonateProjectPlannerOngoingList(searchTitle, page, limit);
			...
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userDonateProjectListService.getDonateProjectPlannerOngoingList(page, limit);
		}
		...
		request.setAttribute("showPage", "user/project/userOngoingDonateProjectList.jsp");

...
public class AdminDeleteService {
	...
	//1. 본인 인증
	public String checkAdminSelf(MemberBean user) {
		...
		MemberBean adminInfo = adminDAO.selectLoginAdmin(user);
	...
	public boolean adminDelete(String id){
		...
		/*-------(insert, update, delete) 성공하면 commit(), 실패하면 rollback() 호출
		 * 		 단, select는 이런 작업을 제외 ------------------*/
		if(deleteAdminCount > 0 && deleteAddrCount > 0) {
			isAdminDeleteResult = true;
			commit(con);
		}else {
			rollback(con);
		}

...
import svc.user.project.UserReadyDonateProjectListService;
import vo.ActionForward;
import vo.PageInfo;
import vo.ProjectPlannerBean;

public class UserReadyDonateProjectListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		UserReadyDonateProjectListService userReadyDonateProjectListService = new UserReadyDonateProjectListService();
		...
		if(selectOrder != null) {//정렬기준으로 조회
			...
			if(!selectOrder.equalsIgnoreCase("default")) {//혹시모를 default 선택 방지
				...
				listCount = userReadyDonateProjectListService.getDonateProjectReadyCount();
				System.out.println("[UserReadyDonateProjectListAction] project_tbl 진행중인 기부 프로젝트 수 = "+listCount);
				
				//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
				projectPlannerList = userReadyDonateProjectListService.getOrderDonateProjectPlannerReadyList(selectOrder, page, limit);
			...
			//project_tbl에서 기부 프로젝트 수를 얻어옴
			listCount = userReadyDonateProjectListService.getSearchDonateProjectReadyCount(searchTitle);
			System.out.println("[UserReadyDonateProjectListAction] project_tbl 검색조건에 따른 진행중인 기부 프로젝트 수 = "+listCount);
			
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userReadyDonateProjectListService.getSearchDonateProjectPlannerReadyList(searchTitle, page, limit);
			...
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userReadyDonateProjectListService.getDonateProjectPlannerReadyList(page, limit);
		}
		...
		request.setAttribute("showPage", "user/project/userReadyDonateProjectList.jsp");

...
public class CancelProjectService {
	...
	/** 진행중인 프로젝트 취소 */
	public boolean cancelOngoingProject(int project_id, ArrayList<RewardBean> rewardList) {
		//1. 커넥션 풀에서 Connection객체를 얻어와
		Connection con = getConnection(); //JdbcUtil. 생략(이유?import static 하여)
	...
	public boolean refundDonation(ArrayList<DonationBean> donationList, int project_id) {
		...
		//[순서-1] 모든 후원금액만큼 후원자 계좌 돈 업데이트 
		int refundDonationCount = 0;
		for(int i=0; i<donationList.size(); i++) {
			DonationBean donation = donationList.get(i);
			
			refundDonationCount += manageProjectDAO.updateUserPlusMoney(donation.getMember_id(), donation.getTotalDonation());
		}
		
		
		//[순서-2] 후원기록을 삭제
		int deleteDonationListCount = manageProjectDAO.deleteDonation(project_id);
		
		boolean isRefundDonationSuccess = false;
		/*-------(insert, update, delete) 성공하면 commit(), 실패하면 rollback() 호출
		 * 		 단, select는 이런 작업을 제외 ------------------*/
		if(refundDonationCount == donationList.size() && deleteDonationListCount > 0) {
			isRefundDonationSuccess = true;
			commit(con);
		}else {
			rollback(con);
		}
		...
		return isRefundDonationSuccess;
	}

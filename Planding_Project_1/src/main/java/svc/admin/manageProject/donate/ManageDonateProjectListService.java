...
public class ManageDonateProjectListService {
	//필드 없음
	...
	//2. 조건에 맞는 회원 수를 가져옴
	public int getSearchDonateCount(String project_title) {
		//1. 커넥션 풀에서 Connection객체를 얻어와
		Connection con = getConnection(); //JdbcUtil. 생략(이유?import static 하여)
	...
	public ArrayList<ProjectBean> getDonateList(int page, int limit) {
		//1. 커넥션 풀에서 Connection객체를 얻어와
		Connection con = getConnection(); //JdbcUtil. 생략(이유?import static 하여)
	...
	public ArrayList<ProjectBean> getOrderDonateList(String order, int page, int limit) {
		//1. 커넥션 풀에서 Connection객체를 얻어와
		Connection con = getConnection(); //JdbcUtil. 생략(이유?import static 하여)

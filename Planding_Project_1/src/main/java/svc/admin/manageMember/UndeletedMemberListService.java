...
public class UndeletedMemberListService {
	...
	public int getSearchUndeletedMemberCount(String member_id) {
		//1. 커넥션 풀에서 Connection객체를 얻어와
		Connection con = getConnection(); //JdbcUtil. 생략(이유?import static 하여)
		...
		/*-------DAO의 해당 메서드를 호출하여 처리----------------------------------------------------*/
		int searchMemberCount = manageMemberDAO.searchDeleteUndeletedMemberCount("N", member_id);
	...
	//3. 정렬기준에 맞는 탈퇴 회원 목록 가져오기
	public ArrayList<MemberBean> getOrderUndeletedMemberList(String order, int page, int limit) {
		//1. 커넥션 풀에서 Connection객체를 얻어와
		Connection con = getConnection(); //JdbcUtil. 생략(이유?import static 하여)
		...
		if(order.equals("new")) { searchMemberList = manageMemberDAO.orderNewDeleteUndeleteMemberList("N", page, limit); }
		if(order.equals("old")) { searchMemberList = manageMemberDAO.orderOldDeleteUndeleteMemberList("N", page, limit); }
		if(order.equals("az")) { searchMemberList = manageMemberDAO.orderAZDeleteUndeleteMemberList("N", page, limit); }
		if(order.equals("za")) { searchMemberList = manageMemberDAO.orderZADeleteUndeleteMemberList("N", page, limit); }

...
import svc.user.project.UserReadyFundProjectListService;
import vo.ActionForward;
import vo.PageInfo;
import vo.ProjectPlannerBean;

public class UserReadyFundProjectListAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		UserReadyFundProjectListService userReadyFundProjectListService = new UserReadyFundProjectListService();
		...
		if(selectOrder != null) {//정렬기준으로 조회
			...
			if(!selectOrder.equalsIgnoreCase("default")) {//혹시모를 default 선택 방지
				...
				listCount = userReadyFundProjectListService.getFundProjectReadyCount();
				System.out.println("[UserReadyFundProjectListAction] project_tbl 진행중인 기부 프로젝트 수 = "+listCount);
				
				//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
				projectPlannerList = userReadyFundProjectListService.getOrderFundProjectPlannerReadyList(selectOrder, page, limit);
			...
			//project_tbl에서 기부 프로젝트 수를 얻어옴
			listCount = userReadyFundProjectListService.getSearchFundProjectReadyCount(searchTitle);
			System.out.println("[UserReadyFundProjectListAction] project_tbl 검색조건에 따른 진행중인 기부 프로젝트 수 = "+listCount);
			
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userReadyFundProjectListService.getSearchFundProjectPlannerReadyList(searchTitle, page, limit);
			...
			//기부 프로젝트-기획자 목록을 얻어옴 (기본값 : 진행중, 최신순)
			projectPlannerList = userReadyFundProjectListService.getFundProjectPlannerReadyList(page, limit);
		}
		...
		request.setAttribute("showPage", "user/project/userReadyFundProjectList.jsp");

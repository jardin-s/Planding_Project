...
public class UserDonateProjectFormAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String planner_id = (String) session.getAttribute("planner_id");
		...
		if(a_id != null) {//관리자 아이디로 후원 시도 시
			...
			if(u_id.equals(planner_id)) {//자신의 프로젝트에 후원하기 시도할 경우 
				response.setContentType("text/html; charset=UTF-8");
				
				PrintWriter out = response.getWriter();
				out.println("<script>");
				out.println("alert('자신의 프로젝트에 후원할 수 없습니다.');");
				out.println("history.back();");
				out.println("</script>");
			}else {

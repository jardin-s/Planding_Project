...
<body>
    ...
    <div class="container-fluid pt-4 mb-5 wow fadeIn" data-wow-delay="0.1s" style="background-color:#86B381">
        <div class="container text-center pt-5">
            <h3 class="display-6 pt-3 mb-4 text-white animated slideInDown">펀딩 프로젝트 관리</h3>
            <div class="row justify-content-center">
	            <ul class="col-12 col-lg-8 nav nav-pills justify-content-center mt-5 mb-0">
					<li class="col-2 nav-item"><a class="nav-link text-white" href="manageFundProjectList.mngp">전체</a></li>
					<li class="col-2 nav-item"><a class="nav-link text-white" href="unauthFundProjectList.mngp">미승인</a></li>
					<li class="col-2 nav-item"><a class="nav-link text-white" href="readyFundProjectList.mngp">공개예정</a></li>
					<li class="col-2 nav-item"><a class="nav-link text-white" href="ongoingFundProjectList.mngp">진행중</a></li>
					<li class="col-2 nav-item"><a class="nav-link active fw-bold" aria-current="page" href="#">종료</a></li>
					<li class="col-2 nav-item"><a class="nav-link text-white" href="successFundProjectList.mngp">성공</a></li>
	            </ul>
            </div>
        </div>
    </div>
    		...
    		<div class="row">
				...
				<div class="col-4 col-md-3">
					<div class="d-flex justify-content-start">
						<form action="doneFundProjectList.mngp" method="post" name="forder">
			    			<select class="form-select py-1" name="selectOrder" id="selectOrder" aria-label="selectOrder" onchange="changeOrder()">
								...
								<c:if test="${orderKeyword eq 'unsent' }"><option value="unsent" selected>미환불 우선</option></c:if>
								<c:if test="${orderKeyword ne 'unsent' }"><option value="unsent">미환불 우선</option></c:if>
	    		...
	    		<div class="col auto">
		    		<div class="d-flex justify-content-end">
		    			<form action="doneFundProjectList.mngp" method="post" name="fsearch">
    		...
    		<div class="container col-10 col-md-6 col-lg-4">
    			<div class="col-12 mb-5">
    				...
    				<c:if test="${searchKeyword eq null }">
    					<p class="text-center">종료된 프로젝트가 없습니다.</p>
    				</c:if>
    			</div>
    		</div>
    	</div>
						...
						<tbody class="table-group-divider">						
							<c:forEach var="project" items="${projectList}">
								<tr class="text-center">
									...
									<td>
										<c:if test="${project.p_status eq 'done'}">
											<button class="btn btn-outline-primary py-1" type="button" id="sendBtn" onclick="location.href='refundAllDonation.mngp?project_id=${project.project_id}'">환불</button>
										</c:if>
										<c:if test="${project.p_status eq 'clear'}">종료</c:if>
									</td>
					...
					<ul class="pagination justify-content-center">
							...
							<c:if test="${pageInfo.page > 1}">
								<c:choose>
									<c:when test="${orderKeyword ne null }">
										<a class="page-link" href="doneFundProjectList.mngp?page=${pageInfo.page -1 }&selectOrder=${orderKeyword}" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
									</c:when>
									<c:when test="${searchKeyword ne null }">
										<a class="page-link" href="doneFundProjectList.mngp?page=${pageInfo.page -1 }&searchTitle=${searchKeyword}" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
									</c:when>
									<c:otherwise>
										<a class="page-link" href="doneFundProjectList.mngp?page=${pageInfo.page -1 }" aria-label="Previous">
											<span aria-hidden="true">&laquo;</span>
										</a>
									</c:otherwise>
								</c:choose>
						...
						<c:forEach var="pNum" begin="${pageInfo.startPage }" end="${pageInfo.endPage}" step="1">
							...
							<c:if test="${pNum ne pageInfo.page }">
								<c:choose>
									<c:when test="${orderKeyword ne null }">
										<li class="page-item"><a class="page-link" href="doneFundProjectList.mngp?page=${pNum}&selectOrder=${orderKeyword}">${pNum}</a></li>
									</c:when>
									<c:when test="${searchKeyword ne null }">
										<li class="page-item"><a class="page-link" href="doneFundProjectList.mngp?page=${pNum}&searchTitle=${searchKeyword}">${pNum}</a></li>
									</c:when>
									<c:otherwise>
										<li class="page-item"><a class="page-link" href="doneFundProjectList.mngp?page=${pNum}">${pNum}</a></li>
									</c:otherwise>
								</c:choose>
							</c:if>
						</c:forEach>
						...
						<li class="page-item">
							...
							<c:if test="${pageInfo.page < pageInfo.maxPage }">
								<c:choose>
									<c:when test="${orderKeyword ne null }">
										<a class="page-link" href="doneFundProjectList.mngp?page=${pageInfo.page +1 }&selectOrder=${orderKeyword}" aria-label="Next">
											<span aria-hidden="true">&raquo;</span>
										</a>
									</c:when>
									<c:when test="${searchKeyword ne null }">
										<a class="page-link" href="doneFundProjectList.mngp?page=${pageInfo.page +1 }&searchTitle=${searchKeyword}" aria-label="Next">
											<span aria-hidden="true">&raquo;</span>
										</a>
									</c:when>
									<c:otherwise>
										<a class="page-link" href="doneFundProjectList.mngp?page=${pageInfo.page +1 }" aria-label="Next">
											<span aria-hidden="true">&raquo;</span>
										</a>
									</c:otherwise>
								</c:choose>
